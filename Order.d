...
struct OrderState(OrderType = SimpleOrder) if (IsOrder !OrderType) {
  string toString() {
    return "OrderState(" ~to !string(order) ~",cumQty=" ~to !string(
        cumQty) ~",volume=" ~to !string(volume) ~",avgPx=" ~to !string(avgPx) ~",leavesQty=" ~to !string(leavesQty) ~")";
  }
  ...
  this(OrderType o) { this.order = o; };

  @property auto avgPx() {
    return cumQty == 0 ? 0.0 : volume / cumQty;
  };

  @property auto leavesQty() {
...
enum buyPred = "a.limitPx < b.limitPx || ( a.limitPx==b.limitPx && "
...
version(unittest) { alias DefaultHandling = VerboseHandling; }
else {
  alias DefaultHandling = NoopHandling;
}

struct OrderManager(OrderType = SimpleOrder, OrderState = OrderState !OrderType,
  ...
  version(diagnostic) {
    void dump() {
      writeln("\n   BUYS\n   ########################################");
      foreach (buy; buys) { writefln("   %s", buy); };

      writeln("\n   SELLS\n   ########################################");
      foreach (sell; sells) { writefln("   %s", sell); };
      write("\n\n");
    };
  };
  ...
  void onOrder(OrderType order) {
    ...
    if (order.side.isBuy) {
      if (sells.empty || order.limitPx < side[0].limitPx) {
        ...
        (order.side.isBuy() ? buys : sells) =
            side[0..idx] ~OrderState(order) ~side[idx..$];
      } else {
        // TODO aggressive buy
      }
    } else {
      ...
      if (buys.empty || order.limitPx > buys[0].limitPx) {
        ...
        while (fillIdx < oppositeSide.length && fillRemainQty > 0) {
          ...
          writefln("fillIdx=%s fillRemain=%s matchOrder=%s", fillIdx,
                   fillRemainQty, matchOrder);
        ...
        auto exec =
            SimpleExecution(totalFillQty, totalFillVolume / totalFillQty);
...
version(unittest) {
  mixin template SimpleBuyTest() {
    ...
    void start() {
      om.onOrder(SimpleOrder(clock, orderId++, SECID, Side.BUY, 25,
                             OrderType.LIMIT, TimeInForce.DAY, 20.0));
    }
  ...
  struct SimpleTest(alias TestType) {
    ...
    void seed(int qty, Side side, PriceType px) {
      ...
      om.onOrder(SimpleOrder(clock, orderId++, SECID, side, qty,
                             OrderType.LIMIT, TimeInForce.DAY, px));
      om.dump();
    }
